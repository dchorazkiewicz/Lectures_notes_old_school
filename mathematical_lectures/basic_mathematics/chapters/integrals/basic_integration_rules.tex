...
\]
on every interval on which the real power \(x^p\) and the displayed antiderivative are defined.
\end{theorembox}

For an arbitrary real exponent, the safest common domain is \(x>0\). Integer powers extend to their usual real domains, while rational powers require their domains to be checked separately.

The condition \(p\ne -1\) is essential. When \(p=-1\), the integrand is \(1/x\), and its integral is logarithmic on any interval not crossing zero:
\[
...
Many integrals become simple only after rewriting roots and fractions as powers. The domain of the original expression must remain visible.
...
The integral was not difficult after the expression was read correctly. The main work was rewriting the integrand into a power without forgetting its domain.
...
\midrule
\(x^p\), \(p\ne -1\) & \(\dfrac{x^{p+1}}{p+1}+C\) on a valid interval \\
\(1/x\) & \(\ln|x|+C\) \\
\(e^x\) & \(e^x+C\) \\
\(\sin x\) & \(-\cos x+C\) \\
\(\cos x\) & \(\sin x+C\) \\
...
A table is useful only after the integrand and its domain have been read. Sometimes the integrand must first be rewritten or transformed.

\begin{summarybox}
When applying basic integration rules, ask:
\begin{itemize}
\item What is the domain of the integrand?
...
\item Can I check the result by differentiating on the stated interval?
\end{itemize}
\end{summarybox}
