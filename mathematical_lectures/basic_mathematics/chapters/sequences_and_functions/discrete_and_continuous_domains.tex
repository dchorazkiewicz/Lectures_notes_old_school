\section{Discrete and interval domains}
...
\subsection*{Interval domains}

Intervals of real numbers are sometimes informally called continuous domains in elementary calculus. This phrase describes the structure of the input set: its points are not isolated. It does not imply that every function defined on the interval is continuous.

\begin{remarkbox}
A domain may be an interval while the function itself has discontinuities. Continuity of a function is a separate property defined using limits.
\end{remarkbox}
...
A function defined on an interval can be studied by asking what happens as \(x\) moves through nearby real inputs. A connected curve is appropriate only when the behavior of the function justifies that picture.
...
When drawing a function defined on an interval, it may be appropriate to draw a connected curve if the function behaves continuously. When drawing a sequence, we should usually draw separated points.
...
\item Am I describing the domain, or continuity of the function?
...
The difference between interval and discrete domains is not cosmetic. It shapes the way we read, draw, and investigate mathematical objects.
