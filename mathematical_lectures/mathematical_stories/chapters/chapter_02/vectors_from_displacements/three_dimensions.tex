...
\]
Written in coordinates, these are the two equations
\[
\begin{aligned}
...
\end{aligned}
\]
We now derive one explicit nonzero solution when the two given vectors are not
parallel.

Multiply the first equation by \(v_3\), multiply the second by \(u_3\), and
subtract. The terms containing \(n_3\) cancel:
\[
\begin{aligned}
&n_1u_1v_3+n_2u_2v_3+n_3u_3v_3\\
&\qquad-
\left(n_1v_1u_3+n_2v_2u_3+n_3v_3u_3\right)=0,
\end{aligned}
\]
so
\[
...
\[
\begin{aligned}
&(u_2v_3-u_3v_2)(u_1v_3-u_3v_1)\\
&\qquad+
(u_3v_1-u_1v_3)(u_2v_3-u_3v_2)=0,
\end{aligned}
\]
because the second factor in the first product is the negative of the first
factor in the second product.

To satisfy the original equations as well, take
\[
n_3=u_1v_2-u_2v_1.
\]
...
\]
Now verify the first perpendicularity condition by expanding every term:
\[
\begin{aligned}
...
\end{aligned}
\]
The same calculation for the second vector gives
\[
\begin{aligned}
...
\end{aligned}
\]
Thus the formula has been constructed from the two right-angle conditions; it
has not been postulated. We now give this derived vector a name:
\[
\boxed{
