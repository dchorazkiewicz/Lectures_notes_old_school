...
\lim_{x\to a}f(x)
\]
exists if and only if, for every \(\varepsilon>0\), there is \(\delta>0\) such that whenever
\[
0<|x-a|<\delta,
\qquad
0<|y-a|<\delta,
...
Heine follows every approaching input sequence. Cauchy compares nearby output values directly. Both formulations express that all admissible approaches must settle into one common behavior.
