...
lines, perpendicularity, congruent segments, angle bisection, and the
...
\begin{itemize}
    ...
    \item bisecting a segment or an angle;
...
A distinction is required between an angle and the numerical measure assigned
to it.

\begin{definitionbox}
An angle with vertex $O$ is determined by two rays with common initial point
$O$, together with the choice of the region swept from the first ray to the
second.
\end{definitionbox}

The angle is a geometric object.  Its measure is a number obtained by comparing
that sweep with a full turn.

\begin{definitionbox}
One full turn is assigned the measure
\[
360^\circ.
\]
If an angle occupies the fraction $r$ of a full turn, then its degree measure
is
\[
m(\alpha)=360^\circ r.
\]
...
angles.  Therefore each of these angles has measure
\[
...
This explains the role of perpendicularity in the Cartesian construction: the
coordinate axes are chosen so that the angle between their positive rays is a
right angle.

\begin{remarkbox}
Later we may express the same angle measure in radians.  A full turn is
$2\pi$ radians, so
\[
90^\circ=\frac{\pi}{2}.
\]
...
\begin{enumerate}
    \item start with Euclidean points, lines, lengths, angles, and perpendicularity;
    \item choose perpendicular axes and a common unit;
    \item measure angle by comparison with a full turn;
    \item assign coordinates by orthogonal projection;
    \item translate Euclidean relations into algebraic formulas.
\end{enumerate}
Coordinates are therefore a numerical representation of the original
Euclidean geometry.
\end{summarybox}
