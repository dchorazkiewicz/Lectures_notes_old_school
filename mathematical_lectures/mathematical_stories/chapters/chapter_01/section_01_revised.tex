...
To measure the opening between the rays, draw a circle centred at $O$. The rays
meet the circle at points $A$ and $B$. The selected angle cuts from the circle
a corresponding arc.

\begin{theorembox}
For every Euclidean circle, the ratio of its circumference $C$ to its diameter
$D$ is the same constant:
\[
\frac{C}{D}=\pi.
\]
The constant is called $\pi$. Since $D=2r$, every circle of radius $r$ has
circumference
\[
\boxed{C=2\pi r}.
\]
...
The independence of $C/D$ from the size of the circle is a consequence of the
similarity of circles: enlarging a circle by a scale factor $\lambda$ multiplies
both its diameter and its circumference by $\lambda$.

The same scaling applies to corresponding arcs. If two circles centred at $O$
have radii $r_1$ and $r_2$, and the same two rays cut arc lengths $s_1$ and
$s_2$, then similarity gives
\[
...
\]
This common ratio depends only on the angle and not on the radius of the circle.

\begin{definitionbox}
Let $s$ be the length of the selected arc on a circle of radius $r$ centred at
the vertex. The radian measure of the angle $\alpha$ is
\[
\boxed{m_{\mathrm{rad}}(\alpha)=\frac{s}{r}}.
\]
This definition is valid because the ratio $s/r$ is independent of the chosen
radius.
\end{definitionbox}

For a full turn, the selected arc is the whole circumference, so
\[
m_{\mathrm{rad}}(\text{full turn})
=
\frac{2\pi r}{r}
=
2\pi.
\]
Thus a half-turn has radian measure $\pi$, and a quarter-turn has radian measure
$\pi/2$.
...
\begin{tikzpicture}[scale=1.0]
    ...
    \draw[helper] (O) -- (2.25,1.299) node[midway,below right] {$r$};
    \node[subset label] at (2.7,1.55) {arc length $s$};
    \fill (O) circle (2pt) node[below left] {$O$};
\end{tikzpicture}
\end{center}

Degrees are a second numerical scale for the same angle. A full turn is assigned
$360^\circ$, so the conversion between the two scales is
\[
...
\subsubsection*{Calculating the measure without measuring an arc}

The arc definition explains what radian measure means, but arc length is often
hard to determine directly. We therefore replace the arc measurement by a
calculation using a triangle.
...
\]
Solving this equation for the angle measure gives the practical formula.
...
\]
The distances are calculated from Cartesian coordinates by the distance formula
derived above.
\end{theorembox}

The formula is independent of how far along the rays the points $A$ and $B$ are
chosen. Moving either point along its ray produces a similar triangle, so the
angle and its measure remain unchanged.
...
\begin{enumerate}
    ...
    \item calculate angle measures from distances by the law of cosines.
\end{enumerate}
