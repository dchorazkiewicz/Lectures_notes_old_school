...
lines, right angles, perpendicularity, congruent segments, circles, arc length,
the Pythagorean theorem, and the law of cosines belong to the Euclidean geometry
...
\begin{itemize}
    ...
    \item constructing a right angle, equivalently constructing a perpendicular
          through a point;
    ...
    \item later, applying the law of cosines in a Euclidean triangle.
\end{itemize}
\end{definitionbox}

A right angle is therefore available from the beginning as a Euclidean
construction. What is postponed until the end of the section is not the
existence of right angles, but their numerical measurement in radians or
degrees.
...
one; equivalently, construct a right angle at $O$. This becomes the $y$-axis.
...
construction. No coordinate formula has yet been used, and no numerical angle
measure has yet been assigned.
...
\begin{remarkbox}
Using one common unit on both axes is essential. It means that a segment of one
unit on the horizontal axis is congruent to a segment of one unit on the
vertical axis. Without a common unit, the later distance formula would not have
its standard Euclidean meaning.
\end{remarkbox}
...
|y_2-y_1|.
\]
Together with the segment $PQ$, these two perpendicular separations form a right
triangle. Here ``right'' means constructed with a right angle; no numerical
measure of that angle is needed for the Pythagorean theorem.
...
This formula is not a new definition of Euclidean distance. It is the coordinate
expression of the Euclidean distance already present in the plane.
...
\begin{enumerate}
    \item start with a Euclidean plane and its geometric constructions, including
    ...
    \item then define general angle measure using circles and arc length;
