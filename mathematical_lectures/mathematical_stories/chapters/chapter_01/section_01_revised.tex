...
\subsection{Angles, Names of Angles, and Angle Measure}

Only now, after the coordinate construction and the distance formula are in
place, do we record how angles are described in the Euclidean plane.
...
An angle is a geometric object. A name such as \emph{right angle} classifies
that object. The measure of the angle is a number assigned to it. These three
notions must not be identified:
\[
\boxed{
\text{angle}
\neq
\text{name of the angle}
\neq
\text{numerical measure of the angle}
}.
\]

\begin{definitionbox}
Let $\alpha$ be an angle. Its degree measure, denoted by $m_{\deg}(\alpha)$,
is defined by comparing the angle with a full turn. A full turn is assigned
measure
\[
360^\circ.
\]
If $\alpha$ occupies the fraction $r$ of a full turn, then
\[
m_{\deg}(\alpha)=360^\circ r.
\]
\end{definitionbox}

Consequently, an angle occupying one half of a full turn has measure
\[
180^\circ,
\]
and an angle occupying one quarter of a full turn has measure
\[
90^\circ.
\]
...
\begin{tikzpicture}[scale=1.0]
    ...
    \node[subset label] at (0.95,0.78) {$m_{\deg}(\alpha)=90^\circ$};
...
angles. Therefore, if $\alpha$ is any one of these right angles, then
\[
m_{\deg}(\alpha)
=
\frac{360^\circ}{4}
...
90^\circ.
\]
Thus a right angle is not the number $90^\circ$; rather, a right angle has
degree measure $90^\circ$.
\end{theorembox}

The coordinate axes were chosen to be perpendicular before coordinates were
assigned. Their positive rays therefore determine a right angle, and that
angle has degree measure $90^\circ$.

\begin{remarkbox}
The same angle may be measured using a different scale. If $\alpha$ is a right
angle, then
\[
...
\]
Degrees and radians are two systems for measuring the same geometric angle;
neither number is the angle itself.
\end{remarkbox}
...
\begin{enumerate}
    ...
    \item finally distinguish an angle, its geometric name, and its numerical
          measure.
\end{enumerate}
