...
\begin{corebox}[When coordinates can be assigned]
Only after the axes, orientations, common unit, refined scale, and real labels
have been constructed can an arbitrary point of the plane receive a coordinate
pair.
\end{corebox}

\begin{derivationbox}[Constructing the two coordinate values]
Let $P$ be any point of the plane. Through $P$, construct the line perpendicular
to the $x$-axis; it meets that axis at a unique point $P_x$. Through $P$,
construct the line perpendicular to the $y$-axis; it meets that axis at a unique
point $P_y$.
\end{derivationbox}
...
\begin{definitionbox}
If the signed real labels of $P_x$ and $P_y$ are $x$ and $y$, respectively, the
coordinates of $P$ are the ordered pair
\[
\boxed{P\longleftrightarrow(x,y)}.
\]
\end{definitionbox}

\begin{proofbox}[Why the correspondence is one-to-one]
...
the corresponding points on the axes and construct through them lines
perpendicular to the respective axes. The two lines meet in exactly one point.
\end{proofbox}
...
\begin{interpretationbox}[Coordinates are addresses]
