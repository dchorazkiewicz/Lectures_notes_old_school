...
\begin{tikzpicture}[scale=0.78]
    ...
    \node at (-3.65,-1.0) {line through two points};

    \coordinate (O) at (0,0);
    \draw[helper] (O) circle (1.25);
    \draw[subset] (O) -- (1.25,0);
    \fill[geometricSubsetColor] (O) circle (2pt);
    \node at (0,-1.75) {circle from centre and radius};

    \coordinate (C) at (3.4,0);
    \coordinate (D) at (5.4,0);
    \draw[helper] (C) circle (1.45);
    \draw[helper] (D) circle (1.45);
    \fill[geometricSubsetColor] (4.4,1.04) circle (2pt) node[above] {$P$};
    \fill[geometricSubsetColor] (4.4,-1.04) circle (2pt) node[below] {$Q$};
    \node at (4.4,-1.75) {new points from intersections};
\end{tikzpicture}
\end{center}
