...
Highlight the difference between \(P(\text{positive}\mid\text{condition})\) and \(P(\text{condition}\mii\text{positive})\). Add a simulation mode that generates individuals and compares empirical proporitions with the exact Bayes calculation. Include a button that changes only prevalence so the student can see why predictive values change even when the test characteristics remain fixed.
\end{quote}
\end{examplebox}
